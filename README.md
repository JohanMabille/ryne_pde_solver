...
Fork the repo (click on the fork button near the top right corner), then clone it on your rynehine
and setup the upstream remote accordingly. For instance, if your github name is JohnDoe:

```bash
git clone https://github.com/JohnDoe/ryne_pde_solver.git
cd ryne_pde_solver
git remote add upstream https://github.com/JohanMabille/ryne_pde_solver.git
```
...
First, be sure you master is up to date; on your remote fork on github (here `JohnDoe/ryne_pde_solver`)
you should see `This branch is even with JohanMabille/ryne_pde_solver`. If not, you need to update:
